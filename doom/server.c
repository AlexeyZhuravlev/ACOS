...
void share_game_to_player(int socket, int n)
{
    ...
    write(socket, &games[n].number_of_players, sizeof(int));
    write(socket, &games[n].max_number_of_players, sizeof(int));
}

void share_increasion(int socket, int n)
{
    write(socket, "I", sizeof(char));
    write(socket, &n, sizeof(int));
}
...
void accept_connections(struct pollfd* server)
{
   if (poll(server, 1, MAX_TIMESPAN))
   {
       ...
       char* client_ip;
       int client_socket = accept(server[0].fd, &client_address, NULL);
    ...
    if (poll(players, n_players, MAX_TIMESPAN))
    {
        ...
        for (i = 0; i < n_players; i++)
            if (players[i].revents == POLLIN)
            {
                ...
                char** words = NULL;
                int j;
                read(players[i].fd, buf, 123);
                parse_words(buf, &words, &n_words);
                if (words[1][0] == 'H')
                {
                    pthread_mutex_lock(&game_mutex);
                    number_of_games++;
                    games = realloc(games, number_of_games * sizeof(struct game_t));
                    ...
                    games[number_of_games - 1].players = malloc(sizeof(struct player_t));
                    games[number_of_games - 1].players[0].name = words[0];
                    games[number_of_games - 1].players[0].socket = players[i].fd;
                    games[number_of_games - 1].max_number_of_players = atoi(words[3]);
                    pthread_mutex_unlock(&game_mutex);
                    for (j = 0; j < n_players; j++)
                        if (players[j].fd > 0)
                            share_game_to_player(players[j].fd, number_of_games - 1);
                    n_hosts++;
                ...
                if (words[1][0] == 'C')
                {
                    ...
                    else
                    {
                        ...
                        games[number_of_game].players = 
                            realloc(games[number_of_game].players, n * sizeof(struct player_t));
                        games[number_of_game].players[n - 1].name = name;
                        games[number_of_game].players[n - 1].socket = players[i].fd;
                        for (j = 0; j < n_players; j++)
                            if (players[j].fd > 0)
                                share_increasion(players[j].fd, number_of_game);
                        for (j = 0; j < games[number_of_game].number_of_players - 1; j++)
        ...
        if (poll(game_players, game.number_of_players, MAX_TIMESPAN))
        {
            for (i = 0; i < game.number_of_players; i++)
                if (game_players[i].revents == POLLIN)
                {
                    ...
                    printf("Player %s left the game %s.", game.players[i].name, game.game_name);
                    alive_players--;
                }
        }
    }
    free(game_players);
    printf("Game %s finished.", game.game_name);
    return NULL;
}
       ...
       for (i = 0; i < n_hosts; i++)
           if (hosts[i].revents == POLLIN)
           {
               ...
               if (decision == 'S')
               {
                   ...
                   if (games[game_number].number_of_players == games[game_number].number_of_players)
                   {
                       int j;
                       pthread_t game;
                       write(hosts[i].fd, "+", sizeof(char));
                       for (j = 0; j < games[game_number].number_of_players; j++)
                           write(games[game_number].players[j].socket, "S", sizeof(char));
                       for (j = 0; j < n_players; j++)
                           if (players[j].fd > 0)
                       ...
                       thread_game_numbers = realloc(thread_game_numbers, n_thread_game_numbers * sizeof(int));
                       thread_game_numbers[n_thread_game_numbers - 1] = game_number;
                       pthread_create(&game, NULL, game_start, &thread_game_numbers[n_thread_game_numbers - 1]);
               ...
               else
               {
                  ...
                  for (j = 0; j < games[game_number].number_of_players; j++)
                  {
                      write(games[game_number].players[j].socket, "C", sizeof(char));
...
int main(int argc, char** argv)
{
    ...
    journal = fopen(journal_name, "w");
    if (journal == NULL)
    {
        printf("Unable to open/create journal file\n");
        return 1;
    }
    dup2(fileno(journal), 1);
    if (argc < 2)
    {
